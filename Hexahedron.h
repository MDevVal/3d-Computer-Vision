...
{
protected:
  QVector4D origin; // origin, controls the distance to the cameras, i.e.
                    // typically it's the bottom-left-front corner
  float dx, dy, dz; // dy = width, dy = height, dz = thickness/depth

public:
  Hexahedron(QVector4D _ori = E0 +
                              E1, // origin in homogeneous coordinates, i.e.
                                  // typically it's the bottom-left-front corner
             float _dx = 1.0,     // width
  ...
  void drawPoints(const RenderCamera &renderer,
                  const QColor &color = COLOR_SCENE, float pointSize = 1) const;
...
protected:
  ...
  constexpr static unsigned edgeList[2 * edgeCount] = {
      0, 1, 0, 3, 1, 2, 2, 3, 4, 5, 4, 7, 5, 6, 6, 7, 0, 4, 1, 5, 2, 6, 3, 7};
  constexpr static unsigned faceList[4 * faceCount] = {
      0, 1, 2, 3, 0, 3, 7, 4, 0, 4, 5, 1, 6, 5, 4, 7, 6, 7, 3, 2, 6, 2, 1, 5};
};
