//
//  This is the camera for the rendering (aka painting objects on the canvas) of
//  the scene ONLY.
//
//  It provides methods to render primitives like points, lines, planes, and
//  point clouds.
//
//  Note: If you need other primitives, you have to add your own render-method
//  here.
//
...
public:
  ...
  // methods to render primitive geometric objects, e.g. points, lines, planes,
  // point clouds, etc.
  ...
  void renderPoint(const QVector4D &p, // render homogeneous point p
                   const QColor &color, float pointSize = 3.0f) const;
  void
  renderLine(const QVector3D &a, // render line between affine points a and b
             const QVector3D &b, const QColor &color,
             float lineWidth = 1.0f) const;
  void renderLine(
      const QVector4D &a, // render line between homogeneous points a and b
      const QVector4D &b, const QColor &color, float lineWidth = 1.0f) const;
  void renderPlane(
      const QVector3D &a, // render plane between affine points a, b, c, d
      const QVector3D &b, const QVector3D &c, const QVector3D &d,
      const QColor &color, float transparency = 0.2f) const;
  void renderPCL(
      const QVector<QVector4D> &pcl, // render point cloud of homogeneous points
      const QColor &color, float pointSize = 3.0f) const;
