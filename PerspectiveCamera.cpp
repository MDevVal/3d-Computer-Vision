...
PerspectiveCamera::PerspectiveCamera(const QVector4D &center,
    ...
    : center(center)
    , pose(pose)
    , imagePrincipalPoint(imagePrincipalPoint)
                                       ...
                                       float lineWidth)
{
    ...
    for (const auto &point : hexahedron) {
        ...
        float numeratorX = camera.pose(0, 0) * dx + camera.pose(1, 0) * dy + camera.pose(2, 0) * dz;
        float denominator = camera.pose(0, 2) * dx + camera.pose(1, 2) * dy
                            + camera.pose(2, 2) * dz;
        float numeratorY = camera.pose(0, 1) * dx + camera.pose(1, 1) * dy + camera.pose(2, 1) * dz;
        ...
        QVector3D projected = QVector3D(camera.center) + x_p * QVector3D(camera.pose.column(0))
                             ...
                             float lineWidth) const
{
    ...
    QVector3D ipp3d = QVector3D(center) + imagePlaneDistance * QVector3D(pose.column(2));
    renderer.renderLine(ipp3d,
                        (ipp3d + 0.5f * QVector3D(pose.column(0))),
                        QColorConstants::Red,
                        4.0f);
    renderer.renderLine(ipp3d,
                        (ipp3d + 0.5f * QVector3D(pose.column(1))),
...
QMatrix4x4 PerspectiveCamera::getPose()
{
    return pose;
}
